...
function showCopyToast() {
  ...
  showCopyToast.timer = window.setTimeout(() => toast.classList.remove("visible"), 1400);
}
...
function initCopyActions() {
  document.addEventListener("click", (event) => {
    ...
    const targetText = targetId ? $(targetId)?.textContent : "";
    copyText(button.dataset.copy || targetText || "");
  });
}

function initExecutiveBrief() {
  const brief = data.executiveBrief;
  $("heroThesis").textContent = brief.thesis;
  $("heroSubhead").textContent = brief.subhead;
  $("heroEditorial").textContent = brief.heroCopy;
  $("executiveGrid").innerHTML = brief.oneMinuteRead.map((item) => `
...
function renderQuoteFilters() {
  const audiences = ["all", ...Array.from(new Set(data.executiveBrief.viralHooks.map((quote) => quote.audience))).sort()];
  $("quoteFilters").innerHTML = audiences.map((audience) => `
    <button class="quote-filter ${audience === state.quoteAudience ? "active" : ""}" type="button" data-audience="${esc(audience)}">
      ${esc(audience)}
    </button>
  `).join("");
  document.querySelectorAll(".quote-filter").forEach((button) => {
    button.addEventListener("click", () => {
      state.quoteAudience = button.dataset.audience;
      renderQuoteKit();
    });
  });
}

function renderQuoteKit() {
  renderQuoteFilters();
  const quotes = data.executiveBrief.viralHooks.filter((quote) => state.quoteAudience === "all" || quote.audience === state.quoteAudience);
  $("quoteGrid").innerHTML = quotes.map((quote) => `
    <article class="quote-card">
      <div class="quote-meta">${esc(quote.audience)} / ${esc(quote.tone)}</div>
      <h3>${esc(quote.hook)}</h3>
      <p><strong>Proof:</strong> ${esc(quote.proof)}</p>
      <p><strong>Caveat:</strong> ${esc(quote.caveat)}</p>
